document.addEventListener("DOMContentLoaded", () => {
  ...
  if (track) {
    ...
    function moveCarousel() {
      slideIndex++;
      track.style.transition = "transform 1s cubic-bezier(0.25, 1, 0.5, 1)";
  ...
  // scrollIntoView ignora scroll-padding — usamos window.scrollTo con offset manual
  ...
  document.querySelectorAll('a[href^="#"]').forEach(anchor => {
    anchor.addEventListener("click", function (e) {
      ...
      const offset   = 32; // margen visual extra para que no quede pegado al header
      const targetY  = target.getBoundingClientRect().top + window.scrollY - headerH - offset;
